...
local function define_tests()
    test.describe("Minesweeper window", function()
        test.it("is a Programs/Games window on the shell SDK with the pack's mine; every picture of the pack is found", function()
            local entry = assert(registry.get("butschster.windows.minesweeper:window"))
            ...
            test.eq(table.concat({meta.type, meta.title, meta.group, meta.image, meta.pixel_render, meta.pixel_state}, "|"),
                "tui_desktop.window|Minesweeper|Programs/Games|butschster.windows.minesweeper:images/mine|butschster.windows.sdk:render|butschster.windows.minesweeper:window")
            test.eq(view.PACK, "butschster.windows.minesweeper:images/")
